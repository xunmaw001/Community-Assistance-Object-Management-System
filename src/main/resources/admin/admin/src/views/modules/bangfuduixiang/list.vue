                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('bangfuduixiang','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/shequbangfuduixiangguanli/upload/bangfuduixiangMuBan.xls"
                        >批量导入帮扶对象数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('bangfuduixiang','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="bangfuduixiangUploadSuccess"
                                :on-error="bangfuduixiangUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('bangfuduixiang','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入帮扶对象数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('bangfuduixiang','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "bangfuduixiang.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
                ...
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('bangfuduixiang','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('bangfuduixiang','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('bangfuduixiang','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            ...
            form:{
                ...
                bangfuduixiangContent : null,
                bangfuduixiangDelete : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'bangfuduixiang',//当前表表名,
                        sumColum : 'bangfuduixiang_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'bangfuduixiang_name',//分组字符串字段
                        // types : 'bangfuduixiang_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "bangfuduixiang_types",
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                params['bangfuduixiangDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "bangfuduixiang/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            bangfuduixiangUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "bangfuduixiang/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            // 导入功能上传文件失败后调用导入方法
            bangfuduixiangUploadError(data){
